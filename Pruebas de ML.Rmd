...
```{r}
vcc_fit <- solo_vcc %>%
        select(Edad, Terapeutica, Operador, Conclusiones1, Conclusiones2, Conclusiones3, TipEs, FecEstudio, Sexo, Sede)

dummy <- as.data.table(dummyVars(" ~ .", data = vcc_fit))
vcc_fit_dummy <- data.frame(predict(dummy, newdata = vcc_fit))

library(data.table)
library(mltools)

vcc_fit_dummy <- one_hot(as.data.table(vcc_fit))
vcc_fit_dummy
